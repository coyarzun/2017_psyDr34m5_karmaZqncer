  ...
  void draw(PGraphics pg) {
    ...
    for (int i=0; i<cols; i++) {
      for (int j=0; j<rows; j++) {
        ...
        float cellsize = 20*(1+in.left.get((i+j*cols)%1024)*environment.audioScaleFactor*2);//
...
class Dust3 extends Dream {
  ...
  void draw(PGraphics pg) {
    ...
    pg.scale(4*(1+environment.audioScaleFactor)*in.left.get(0));
...
class Dust4 extends Dream {
  ...
  void draw(PGraphics pg) {
    ...
    pg.rotateZ(-PI/2);
    pg.rotateX(radians(frameCount*1*1));
    pg.scale(1*(1+environment.audioScaleFactor)*in.left.get(0));
