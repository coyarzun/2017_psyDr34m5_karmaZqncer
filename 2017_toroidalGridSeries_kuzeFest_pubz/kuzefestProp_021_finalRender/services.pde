...
void scaleGuides(){
  //guidefactor
  for(int i=0; i<vguides.length; i++){vguides[i].from.x*=guidefactor;vguides[i].to.x*=guidefactor;}
  for(int i=0; i<hguides.length; i++){hguides[i].from.y*=guidefactor;hguides[i].to.y*=guidefactor;}
}
void sortGuides() {
}
...
int maxframes = 20*30, cframe, take = 0;
String recName;
...
void startRecording(){
  ...
  cframe = 0;
  take++;
  recName = "_render/animationTest/take_"+nf(take,3)+"_";
}
void stopRecording(){
  recording = false;
}
